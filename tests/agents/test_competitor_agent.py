...
async def test_competitor_agent_contract(idea):
    agent = CompetitorAgent(llm=FakeLLMProvider())
    result = await agent.run(idea)
    assert isinstance(result, AgentResult)
    assert result.agent_name == "competitor"
    assert result.status in set(AgentStatus)
    assert 0.0 <= result.confidence <= 1.0
    assert isinstance(result.evidence, list)


async def test_competitor_agent_runs_without_dependencies(idea):
    result = await CompetitorAgent().run(idea)
